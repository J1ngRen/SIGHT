# test_gcn_goodnode_robustness_final.py — adds Combined (ID+OOD) Uncorr AUROC; others unchanged
import os
import json
import argparse
import time
...
def compute_uncertainty_metrics(model, X_spikes, data, mask, num_classes, task_type, T_cal=None):
    ...
    # use ID validation split for calibration
...
if __name__ == "__main__":
    ...
    parser.add_argument("--shift", type=str, default="covariate", help="no_shift / covariate / concept")
    ...
    for seed in seeds:
        ...
        if task_type == "Binary classification":
            labels_all = data.y.view(-1).long()
            invalid = torch.zeros_like(labels_all, dtype=torch.bool)
        else:
            labels_all, invalid = get_multiclass_index_labels(data.y, C)
        ...
        if task_type != "Binary classification":
            combined_mask = combined_mask & (~invalid)
